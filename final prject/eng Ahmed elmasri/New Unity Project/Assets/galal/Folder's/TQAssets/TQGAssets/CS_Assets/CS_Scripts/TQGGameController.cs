...
namespace TriviaQuizGame
{
	...
	public class TQGGameController:MonoBehaviour 
	{
        ...
        public GameObject exelant,good,verygood;
			...
			if ( isGameOver == false )
			{
				...
				else  
				{
					...
					if ( dontRepeatQuestions == true && currentQuestion >= questions.Length )    
					{
						...
						questionObject.Find("Text").GetComponent<Text>().text = " كل الاسئله يجب ان تحل, \nاعاده الاسئه ... ";
		...
		IEnumerator GameOver(float delay)
		{
			...
			if ( gameOverCanvas )    
			{
				...
				if ( gameOverCanvas.Find("ScoreTexts/TextScore") )    gameOverCanvas.Find("ScoreTexts/TextScore").GetComponent<Text>().text = "الدرجه " + players[currentPlayer].score.ToString();
				...
				gameOverCanvas.Find("ScoreTexts/TextHighScore").GetComponent<Text>().text = "اعلى درجه " + highScore.ToString();
		...
		IEnumerator Victory(float delay)
		{
			...
			if ( victoryCanvas )    
			{
				...
				{
					//Write the score text, if it exists
					victoryCanvas.Find("ScoreTexts/TextScore").GetComponent<Text>().text = "الدرجه " + players[currentPlayer].score.ToString();
                    //exelant.text = "";
                    //cub.enabled = false;
                    //Check if we got a high score
                    if (players[currentPlayer].score >= 5 && players[currentPlayer].score <= 35)
                    {
                        good.SetActive(true);
                    }
                    else if (players[currentPlayer].score >= 35 && players[currentPlayer].score < 70)
                    {
                        verygood.SetActive(true);
                    }
                    else if (players[currentPlayer].score >= 70 && players[currentPlayer].score <= 80)
                    {
                        exelant.SetActive(true);
                        cub.SetActive(true);
                    }
					...
					//Write the high sscore text
					victoryCanvas.Find("ScoreTexts/TextHighScore").GetComponent<Text>().text = "اعلى درجه " + highScore.ToString();
				}
				...
				if ( victoryCanvas.Find("ScoreTexts/Players") )
				{
					...
					else
					{
						...
						victoryCanvas.Find("TextResult").GetComponent<Text>().text = "رسم بين  ";
						...
						while ( winnerCount > 0 )
						{
							...
							if ( winnerCount == 0 )    victoryCanvas.Find("TextResult").GetComponent<Text>().text += "و " + players[winnerCount].name;
							else    victoryCanvas.Find("TextResult").GetComponent<Text>().text += players[winnerCount].name + ", ";
						}
						
						// Display the score they got
						victoryCanvas.Find("TextResult").GetComponent<Text>().text += " كل ب " + players[0].score.ToString() + " نقط!";
					}
				}
				...
				if ( victoryCanvas.Find("ScoreTexts/TextProgress")  )
				{
                    ...
                    if (correctAnswers >= 5&& correctAnswers <=6)
                    {
                        good.SetActive(true);
                    }
		...
		public void SetNumberOfPlayers( float setValue )
		{
			...
			if ( startCanvas && startCanvas.Find("NumberOfPlayers/HandleSlideArea/Handle/Text") )    startCanvas.Find("NumberOfPlayers/HandleSlideArea/Handle/Text").GetComponent<Text>().text = numberOfPlayers.ToString() + " طالب";
